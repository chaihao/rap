    ...
    public function up(): void
    {
        ...
        if (!Schema::hasTable($tableNames['permissions'])) {
            Schema::create($tableNames['permissions'], function (Blueprint $table) {
                ...
                $table->string('uri')->nullable()->comment('URI 路径');
                $table->string('controller')->nullable()->comment('控制器');
                $table->string('action')->nullable()->comment('控制器操作');
                $table->string('slug')->nullable()->comment('别名');
                $table->string('prefix')->nullable()->comment('路由前缀');
                $table->string('guard_name'); // For MyISAM use string('guard_name', 25);
                $table->json('middleware')->nullable()->comment('中间件');
                $table->string('group')->nullable()->comment('分组');
                $table->string('group_name')->nullable()->comment('分组名称');
                $table->boolean('is_login')->default(false)->comment('是否需要登录');
